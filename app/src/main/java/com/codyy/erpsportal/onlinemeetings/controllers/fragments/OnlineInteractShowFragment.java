...
public class OnlineInteractShowFragment extends OnlineFragmentBase{
    ...
    @Bind(R.id.text_doc_title)TextView mTitleTextView;
    @Bind(R.id.text_btn_more)ImageView mMoreImageView;
    @Bind(R.id.drawer_layout_online_show)DrawerLayout mDrawerLayout;
    @Bind(R.id.recycler_view_online_show)RecyclerView mRecyclerView;//文档列表。
    @Bind(R.id.rlt_container)RelativeLayout mContainer;//容器
    ...
    private void refreshCurrentContent() {
        ...
        if (mList != null && mList.size() > 0) {
            ...
            loadContent(mList.get(index),false);
        }else{
...
//        private String cocoAddress = "http://10.1.70.15:8080/ResourceServer/res/view/download/doc/a4d33cc6c958498ba18ca70c08f3bbdd/cf3722cd5c7c4012b7d4e8921bd69303.do?showName=COCO\\U8868\\U60c5\\U5b9a\\U4e49.doc";
        ...
        if(isPause){
            ...
            if(mCurrentShow!=null && UiOnlineMeetingUtils.DOC_TYPE_PDF.equals(UiOnlineMeetingUtils.CheckResourceType(mCurrentShow.getShowTitle()))){
                if(!mIsShowFromUser){//防止一次演示未完成被终止。
                    loadContent(mCurrentShow,false);
                }else{
    ...
    public void setDocSelected(int index ,boolean isNewOpen){
        ...
        //加载数据 .
        loadContent(mList.get(index), isNewOpen);
    }
        ...
        if(pos>-1){
            ...
            if(mList.size() == 0) {
                ...
                if(mList.size()>pos){
                    ...
                    loadContent(mList.get(pos),false);
                }else {
                    loadContent(mList.get(mList.size()-1),false);
                }
            }
