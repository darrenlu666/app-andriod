...
public class ResourcesNewActivity extends AppCompatActivity implements Callback{
     ...
     * 上传的资源
     */
    public static final String FROM_SELF = "UPLOAD";
    /**
     * 下级推荐的资源
     */
    public static final String FROM_RECOMMENDED = "RECOMMEND";
    ...
    private OnItemClickListener mOnItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(int position) {
            ...
            if (object instanceof Video){
                ...
                VideoDetailsActivity.start(ResourcesNewActivity.this, mUserInfo, video.getId());
            } else if (object instanceof Audio) {
                List<Audio> audios = mAdapter.getData();
                AudioDetailsActivity.start(ResourcesNewActivity.this, mUserInfo, audios, position);
            } else if (object instanceof Document) {
                Document document = (Document) object;
                DocumentContentActivity.start(ResourcesNewActivity.this, mUserInfo, document);
            } else if (object instanceof Image) {
                Image image = (Image) object;
                ImageDetailsActivity.start(ResourcesNewActivity.this, mUserInfo, image.getId());
            }
        }
    };
...
//        mAudioManager = (AudioManager) getSystemService(AUDIO_SERVICE);
        ...
        mUrl = URLConfig.RESOURCE_NEW_LIST;
    }
...
//        mAudioControlBar.setCallback(mControlBarCallback);
        ...
        mFilterTypeMenuController = new Builder().setActivity(this)
                ...
                .setOnMenuClickListener(new OnMenuClickListener() {
                    ...
                    public void onKnowledgeBtnClick() {
                        ResourceKnowledgeFilterAct.startForResult(ResourcesNewActivity.this, new AreaInfo(mUserInfo));
                    }
                })
                .build();
    ...
    private void updateCurrentScopeTv() {
        ...
        RadioButton fromRb = (RadioButton) mFromRg.findViewById(mFromRg.getCheckedRadioButtonId());
        sb.append(fromRb.getText());
        RadioButton typeRb = (RadioButton) mTypeRg.findViewById(mTypeRg.getCheckedRadioButtonId());
        sb.append(" • ").append(typeRb.getText());
     ...
     * 根据用户类型不同而不同
     */
    private void initFromFilterItems() {
        mFrom = FROM_SELF;
        addFromFilterItem(Titles.sWorkspaceResourceUpload, FROM_SELF);
        if (!mUserInfo.isStudent()) {//只要不是学生，都有下级推荐
            addFromFilterItem(Titles.sWorkspaceResourceRecommend, FROM_RECOMMENDED);
        }
        if (mUserInfo.isStudent() || mUserInfo.isTeacher()) {//老师或学生要显示收藏的资源
//            addFromFilterItem("我的专辑", FROM_ALBUM);
            addFromFilterItem(Titles.sWorkspaceResourceCollect, FROM_FAVORITE);
        }
        mFromRg.check(mFromRg.getChildAt(0).getId());
    }
    ...
    private void onFromFilterItem(View view) {
        ...
        if (mFrom.equals(tag)) return;
        String tagStr = (String) tag;
        switch (tagStr) {
            case FROM_SELF:
            case FROM_RECOMMENDED:
            case FROM_FAVORITE:
                mUrl = URLConfig.RESOURCE_NEW_LIST;
                //noinspection WrongConstant
                mFrom = tagStr;
                mOrderFilterItemsSv.setVisibility(View.VISIBLE);
                loadData(true);
                break;
            case FROM_ALBUM:
                mUrl = URLConfig.RESOURCE_NEW_LIST;
                mFrom = FROM_ALBUM;
                mOrderFilterItemsSv.setVisibility(View.GONE);
                loadData(true);
                break;
        }
        updateCurrentScopeTv();
    ...
    protected void addParams(Map<String, String> params) {
        ...
        if (FROM_ALBUM.equals(mFrom)) {
            params.remove("ResOriginType");
        } else {
            params.put("ResOriginType", mFrom);
        }
        if (TYPE_ALL.equals(mType)) {
            params.remove("resType");
        } else {
            params.put("resType", mType);
        }
        if (TextUtils.isEmpty(mOrder)) {
            params.remove("orderBy");
        } else {
            params.put("orderBy", mOrder);
        }
    }
    ...
    @StringDef({FROM_SELF, FROM_RECOMMENDED, FROM_FAVORITE, FROM_ALBUM})
    @interface ResourceFrom { }
}
