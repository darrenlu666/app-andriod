...
import com.codyy.erpsportal.commons.controllers.viewholders.RemoteDirectorViewHolder;
 ...
 */
public class RemoteDirectorListActivity extends AppCompatActivity implements View.OnClickListener, PullToRefreshBase.OnRefreshListener2, AdapterView.OnItemClickListener {
    private final static String EXTRA_USER_INFO = "user_info";
    private String TAG = "RemoteDirectorListActivity";
    private PullToRefreshListView mListView;
    private EmptyView mEmptyView;
    private ObjectsAdapter<RemoteDirectorItem, RemoteDirectorViewHolder> mAdapter;
    ...
    private void initView() {
        ...
        mAdapter = new ObjectsAdapter<>(this, RemoteDirectorViewHolder.class);
        TextView tvTitle = (TextView) findViewById(R.id.tv_title);
        tvTitle.setText(TextUtils.isEmpty(Titles.sWorkspaceSpeclassBroadcast) ? "" : Titles.sWorkspaceSpeclassBroadcast);
    ...
    private void loadData(final boolean refresh) {
        ...
        params.put("remote_switch", "Y");
        params.put("directorUserId", mUserInfo.getBaseUserId());
        params.put("schoolId", "");
        ...
        mRequestSender.sendRequest(new RequestSender.RequestData(URLConfig.GET_DIRECTOR_LIST, params, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                ...
                if ("success".equals(response.optString("result"))) {
                    ...
                    if (total == 0) {
                        ...
                        List<RemoteDirectorItem> remoteDirectorItemListList = RemoteDirectorItem.parseJsonArray(jsonArray);
                        if (refresh) {
                            mAdapter.setData(remoteDirectorItemListList);
                        } else {
                            mAdapter.addData(remoteDirectorItemListList);
                        }
                        mAdapter.notifyDataSetChanged();
                        mListView.onRefreshComplete();
                        //如果已经加载所有，下拉更多关闭
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        showDialog();
        RemoteDirectorItem remoteDirectorItem = mAdapter.getItem(position - 1);
        fetchRemoteDirectorConfig(remoteDirectorItem.getAppointmentId(),
                remoteDirectorItem.getSubjectName(),
                remoteDirectorItem.getSpeakerUserName(),
                "PROGRESS".equals(remoteDirectorItem.getStatus()));
    }
            ...
            public void onResponse(JSONObject jsonObject) {
                ...
                if ("success".equals(result)) {
                    ...
                    if (TextUtils.isEmpty(config.getPmsRemoteUrl())) {
                        ...
                        RemoteDirectorActivity.start(RemoteDirectorListActivity.this, config, subject, teacher, isProgressing);
                    }
    ...
    public static void start(Activity activity, UserInfo userInfo) {
        Intent intent = new Intent(activity, RemoteDirectorListActivity.class);
