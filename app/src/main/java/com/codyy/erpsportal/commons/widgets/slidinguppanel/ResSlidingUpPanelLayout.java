package com.codyy.erpsportal.commons.widgets.slidinguppanel;
...
public class ResSlidingUpPanelLayout extends ViewGroup {

    private static final String TAG = ResSlidingUpPanelLayout.class.getSimpleName();
    ...
    /**
     * True if the collapsed panel should be dragged up.The gravity is bottom.
     */
    ...
    private View mContentScrollableView;

    public void setContentScrollableView(View contentScrollableView) {
        this.mContentScrollableView = contentScrollableView;
    }
    ...
    public ResSlidingUpPanelLayout(Context context) {
        this(context, null);
    }

    public ResSlidingUpPanelLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ResSlidingUpPanelLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mScroller = ScrollerCompat.create(getContext(), scrollerInterpolator);
    ...
    private boolean mInterceptOnContent;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (isViewUnder(mContentScrollableView, (int) mInitialMotionX, (int) mInitialMotionY)) {
                    Cog.d(TAG, "onInterceptTouchEvent ady=", ady
                            , ",bottom=", mSlideableView.getBottom()
                            , ",mPanelHeight=", mPanelHeight);
                    if (ady > adx && dy < 0 && !mIsSlidingUp && mSlideableView.getBottom() > mPanelHeight) {
                        Cog.d(TAG, "onInterceptTouchEvent intercept");
                        mInterceptOnContent = true;
                        return true;
                    }
                }
    ...
    private float mInitialPanelTop;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mInterceptOnContent) {
            float dy = 0;
            Cog.d(TAG, "onTouchEvent heihei ev=", ev);
            final int action = MotionEventCompat.getActionMasked(ev);
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mLastMotionY == 0) {
                        mInitialPanelTop = mSlideableView.getTop();
                        dy = ev.getY() - mInitialMotionY;
                    } else {
                        dy = ev.getY() - mLastMotionY;
                    }
                    mLastMotionY = ev.getY();
                    if (dy < 0) {
                        float currTop = mInitialPanelTop + ev.getY() - mInitialMotionY;
                        if (mDragHelper.smoothSlideViewTo(mSlideableView, mSlideableView.getLeft(), (int)currTop)) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    Cog.d(TAG, "onTouchEvent yVelocity=", mVelocityTracker.getYVelocity());
//                    mScroller.fling(0, mSlideableView.getTop()
//                            , (int) mVelocityTracker.getXVelocity(), (int)mVelocityTracker.getYVelocity()
//                            , 0, 0
//                            , (int) mInitialPanelTop, getPaddingTop() - mSlideableView.getMeasuredHeight() + mPanelHeight);
...
//            Cog.d(TAG, "computeScroll start currY=", mScroller.getCurrY());
//            if (mDragHelper != null && mDragHelper.smoothSlideViewTo(mSlideableView,
//                    mSlideableView.getLeft(), mScroller.getCurrY())) {
//                Cog.d(TAG, "computeScroll currY=", mScroller.getCurrY());
//                setAllChildrenVisible();
//                needInvalidate = true;
//            }
//        }
//        if (needInvalidate) ViewCompat.postInvalidateOnAnimation(this);
    }
