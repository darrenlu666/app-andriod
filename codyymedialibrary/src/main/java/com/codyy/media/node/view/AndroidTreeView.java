package com.codyy.media.node.view;
...
import com.codyy.media.node.holder.SimpleViewHolder;
...
public class AndroidTreeView {
    ...
    public <E> List<E> getSelectedChildValues(Class<E> clazz) {
        List<E> result = new ArrayList<>();
        List<TreeNode> selected = getSelectedChild();
        for (TreeNode n : selected) {
            Object value = n.getValue();
            if (value != null && value.getClass().equals(clazz)) {
                result.add((E) value);
    ...
    private List<TreeNode> getSelectedChild() {
        if (mSelectionModeEnabled) {
            return getSelectedChild(mRoot, true);
        } else {
            return new ArrayList<>();
        }
    }

    private List<TreeNode> getSelectedChild(TreeNode parent, boolean isRoot) {
        List<TreeNode> result = new ArrayList<>();
        for (TreeNode n : parent.getChildren()) {
            if (!isRoot && n.isSelected()) {
                result.add(n);
            }
            result.addAll(getSelectedChild(n, false));
        }
        return result;
    }
    ...
    public void selectNodeParent(TreeNode parent, boolean selected, boolean skipCollapsed) {
        int isNotAll = 0;
        for (TreeNode node : parent.getChildren()) {
            if (node.isSelected()) {
                isNotAll++;
