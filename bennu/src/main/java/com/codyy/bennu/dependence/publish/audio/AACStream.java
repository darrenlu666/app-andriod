...
package com.codyy.bennu.dependence.publish.audio;
...
import com.codyy.bennu.dependence.publish.SessionBuilder;
import com.codyy.bennu.dependence.publish.hw.CodecManager;
import com.codyy.bennu.dependence.publish.rtmp.RTMPAACADTSPacketizer;
import com.codyy.bennu.dependence.publish.rtmp.RTMPAACLATMPacketizer;
import com.codyy.bennu.dependence.publish.rtmp.RTMPClient;
import com.codyy.bennu.dependence.publish.rtmp.RTMPSoftAACPacketizer;
import com.codyy.bennu.dependence.publish.rtp.MediaCodecInputStream;
...
public class AACStream extends AudioStream {
	...
	//added by leitiannet 2014/11/14
	public void setObserver(RTMPClient observer)
	{
		if (mMode == MODE_MEDIARECORDER_API) 
		{
			((RTMPAACADTSPacketizer)mPacketizer).setObserver(observer);
		} 
		else if (mMode == MODE_MEDIACODEC_API)
		{ 
			((RTMPAACLATMPacketizer)mPacketizer).setObserver(observer);
		}
		else if (mMode == MODE_LIBFAAC_API)
		{
			((RTMPSoftAACPacketizer)mPacketizer).setObserver(observer);
		}
	...
	public synchronized void configure() throws IllegalStateException, IOException 
	{
		...
		mRequestedMode = MODE_LIBFAAC_API;
		if (mMode != mRequestedMode || mPacketizer==null) 
		{
			mMode = mRequestedMode;
			if (mMode == MODE_MEDIARECORDER_API) 
			{
				//modified by leitiannet
				//mPacketizer = new AACADTSPacketizer();
				mPacketizer = new RTMPAACADTSPacketizer();
				//end 
			} 
			else if (mMode == MODE_MEDIACODEC_API)
			{ 
				...
				mPacketizer = new RTMPAACLATMPacketizer();
				//end 
			}
			else if (mMode == MODE_LIBFAAC_API)
			{
				mPacketizer = new RTMPSoftAACPacketizer();
			}
...
//=========================================================
	...
	protected void encodeWithLibFAAC() throws IOException 
	{
		
	}
	
	class FAACAudioRecordThread implements Runnable 
	{  
		@Override  
		public void run() 
		{  
	...
	void calcInterval(int len)
	{
		...
		lasttime = currenttime;
		Log.d("ll", String.format("AudioRecordThread() delta=%d len=%d", delta, len));
	}
	...
	{
		CodecManager.findEncodersForMimeType("audio/mp4a-latm");
		//是否需要*2？
		...
		//mMediaCodec.release();
		MediaFormat format = MediaFormat.createAudioFormat("audio/mp4a-latm", mQuality.samplingRate, 1);
		format.setString(MediaFormat.KEY_MIME, "audio/mp4a-latm");
		format.setInteger(MediaFormat.KEY_BIT_RATE, mQuality.bitRate);
		format.setInteger(MediaFormat.KEY_AAC_PROFILE, MediaCodecInfo.CodecProfileLevel.AACObjectLC);
		format.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, bufferSize);//leitiannet 改成一半
		mMediaCodec = MediaCodec.createEncoderByType("audio/mp4a-latm");
		//mMediaCodec = MediaCodec.createByCodecName("AACEncoder");//OMX.google.aac.encoder
		...
		//leitiannet
		((RTMPAACLATMPacketizer)mPacketizer).setAudioParams(mQuality.samplingRate);	
		...
		mThread = new Thread(new Runnable() {
			...
			public void run()
			{
				...
				{
					//
					//((RTMPAACLATMPacketizer)mPacketizer).setMediaCodec(mMediaCodec);	
					//mPacketizer.start();
					...
					{	
						...
						len = mAudioRecord.read(audiobuf, 0, bufferSize);
						...
						bufferIndex = mMediaCodec.dequeueInputBuffer(0);//10000
						if (bufferIndex>=0) //
						{
							inputBuffers[bufferIndex].clear();
							inputBuffers[bufferIndex].put(audiobuf);
							mMediaCodec.queueInputBuffer(bufferIndex, 0, len, System.nanoTime()/1000, 0);
							
							//int count = readAndSendFrame(mMediaCodec);
							
//							long duration = (System.nanoTime() - start)/1000000;
//							Log.d("ll", String.format("onPreviewFrame() duration=%d count=%d", duration, count));
////							if (duration < 23)
////							{
////								try {
...
////							}
	...
	public int readAndSendFrame(MediaCodec mediaCodec)
	{
		int count = 0;
		...
		{  
			if (bufferInfo.size != 0)
			{
				...
				((RTMPAACLATMPacketizer)mPacketizer).sendEncodedData(outData, outData.length, bufferInfo.presentationTimeUs/1000);
