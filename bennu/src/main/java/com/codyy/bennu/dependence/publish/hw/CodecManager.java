...
public class CodecManager {
	...
	public synchronized static Codec[] findEncodersForMimeType(String mimeType) 
	{
		...
		Log.d("CodecManager", String.format("codecCount=%d", codecCount));
		// We loop through the encoders, apparently this can take up to a sec (testes on a GS3)
		for(int j = codecCount - 1; j >= 0; j--)
		{
			MediaCodecInfo codecInfo = MediaCodecList.getCodecInfoAt(j);
			String[] types = codecInfo.getSupportedTypes();
			Log.d("CodecManager", "codecName=" + codecInfo.getName() + " isEncoder=" + codecInfo.isEncoder() + " typeCount=" + types.length);
