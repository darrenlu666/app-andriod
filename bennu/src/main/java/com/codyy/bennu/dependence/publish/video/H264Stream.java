...
import com.codyy.bennu.dependence.publish.exceptions.StorageUnavailableException;
import com.codyy.bennu.dependence.publish.hw.EncoderDebugger;
import com.codyy.bennu.dependence.publish.mp4.MP4Config;
...
public class H264Stream extends VideoStream {
	...
	public void setObserver(RTMPClient observer)
	{
		...
		{
			((RTMPH264MediaRecoderPacketizer)mPacketizer).setObserver(observer);
		} 
		else 
		{ 
			((RTMPH264Packetizer)mPacketizer).setObserver(observer);
		}
	...
	public H264Stream(int cameraId) {
		...
		//mPacketizer = new RTMPH264Packetizer();
	}
	...
	public synchronized void start() throws IllegalStateException, IOException {
		if (!mStreaming) {
			...
			if (mMode == MODE_MEDIARECORDER_API)
			{
				...
				((RTMPH264MediaRecoderPacketizer)mPacketizer).setVideoConf(sps, pps);	
			}
			//end
	...
	public synchronized void configure() throws IllegalStateException, IOException {
		...
		if (mMode == MODE_MEDIARECORDER_API) 
		{
			mPacketizer = new RTMPH264MediaRecoderPacketizer();
		} 
		else 
		{ 
			mPacketizer = new RTMPH264Packetizer();
...
//			return new MP4Config(debugger.getB64SPS(), debugger.getB64PPS());
			EncoderDebugger.debug(mSettings, mQuality.resX, mQuality.resY);
