...
package com.codyy.bennu.dependence.publish.hw;
import java.nio.ByteBuffer;

import android.media.MediaCodecInfo;
import android.util.Log;

import com.codyy.bennu.dependence.publish.rtmp.NativeMemory;
...
public class NV21Convertor {
	...
	private final static String TAG = "NV21Convertor";
	...
	public byte[] convert(byte[] data) 
	{
		...
		if (!mPlanar) 
		{
			...
			{	
				...
				{
					nativeMemory = new NativeMemory(mSize);
				}
				...
				if (!mPanesReversed) 
				{
					swapData = nativeMemory.swapAdjacentTwo(data, mSize);
...
//					}
				...
				long spendReversed = (System.nanoTime() - startReversed)/1000000;
				
				long startYPadding = System.nanoTime();
				if (mYPadding>0) 
				{
//					System.arraycopy(data, 0, mBuffer, 0, mSize);
//					System.arraycopy(data, mSize, mBuffer, mSize+mYPadding, mSize/2);
					System.arraycopy(swapData, 0, mBuffer, 0, mSize);
					System.arraycopy(swapData, mSize, mBuffer, mSize+mYPadding, mSize/2);
					return mBuffer;
				}
				long spendYPadding = (System.nanoTime() - startYPadding)/1000000;
				//Log.d(TAG, String.format("mPlanar=false spendReversed=%d spendYPadding=%d mSize=%d", spendReversed, spendYPadding, mSize));
		...
		else 
		{
			if (mSliceHeight==mHeight && mStride==mWidth) 
			{
				...
				long spendYPadding = (System.nanoTime() - startYPadding)/1000000;
				Log.d(TAG, String.format("mPlanar=true spendReversed=%d spendYPadding=%d", spendReversed, spendYPadding));
