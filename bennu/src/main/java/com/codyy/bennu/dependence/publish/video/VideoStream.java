...
package com.codyy.bennu.dependence.publish.video;
...
import com.codyy.bennu.dependence.publish.MediaStream;
import com.codyy.bennu.dependence.publish.Stream;
import com.codyy.bennu.dependence.publish.exceptions.CameraInUseException;
import com.codyy.bennu.dependence.publish.exceptions.ConfNotSupportedException;
import com.codyy.bennu.dependence.publish.exceptions.InvalidSurfaceException;
import com.codyy.bennu.dependence.publish.gl.SurfaceView;
import com.codyy.bennu.dependence.publish.hw.EncoderDebugger;
import com.codyy.bennu.dependence.publish.hw.NV21Convertor;
import com.codyy.bennu.dependence.publish.rtmp.RTMPH264MediaRecoderPacketizer;
import com.codyy.bennu.dependence.publish.rtmp.RTMPH264Packetizer;
...
public abstract class VideoStream extends MediaStream {
	...
	protected void encodeWithMediaRecorder() throws IOException, ConfNotSupportedException {
		...
		//leitiannet
		((RTMPH264MediaRecoderPacketizer)mPacketizer).setVideoParams(mRequestedQuality.resX, mRequestedQuality.resY, mRequestedQuality.framerate);	
		//end
	...
	long firstFrameStart  = 0;
	long firstFrameEncode = 0;
	long mediaCodecInitTime = 0;
	
	long frameNo = 0;
	
	boolean firstVideo = true;
	byte[] callbackBuffer = null;
	int callbackBufferSize = 0;
	private boolean convertColorFormat = false;
	@SuppressLint("NewApi")
	protected void encodeWithMediaCodecMethod1() throws RuntimeException, IOException {
		...
		colorFormat = debugger.getEncoderColorFormat();

		//获取软编码器时不需要进行颜色转换，硬件编码器时需要颜色转换
		...
		mediaFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, 1);// 1
		mMediaCodec.configure(mediaFormat, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
		mMediaCodec.start();
		
		//leitiannet
		((RTMPH264Packetizer)mPacketizer).setVideoParams(mQuality.resX, mQuality.resY, mQuality.framerate);	
		interval = 1000/mQuality.framerate;//ms
		min_interval = interval / 2;
		max_interval = interval * 3 / 2;
		
		callbackBufferSize = convertor.getBufferSize();
		callbackBuffer = new byte[callbackBufferSize];
		convertColorFormat = convertColorFormatByManufacturer();
		((RTMPH264Packetizer)mPacketizer).setVideoBufferSize(callbackBufferSize);
		//end

		Camera.PreviewCallback callback = new Camera.PreviewCallback() 
		{
			long now = System.nanoTime()/1000, oldnow = 0, i=0;
			...
			{				
				try 
				{
					...
					// 注意:不能在这里进行颜色空间转换，否则影响采集。使用魅族3测试，正常delta为40-43ms，加上颜色空间转换则变成82ms左右
					// 小米手机反而效果好 30-50
					//convertor.convert(data);
					((RTMPH264Packetizer)mPacketizer).putData(data, data.length, System.nanoTime(), delta);
...
//		//mPacketizer.setInputStream(new MediaCodecInputStream(mMediaCodec));
		((RTMPH264Packetizer)mPacketizer).setEncoder(mMediaCodec, convertor);	
		mPacketizer.start();

		mStreaming = true;
	}
	
	protected boolean convertColorFormatByManufacturer() 
	{
    ...
    long calcInterval()
	{
		...
		lasttime = currenttime;
		//Log.d("ll", "onPreviewFrame() delta=" + delta);
		return delta;
	}
...
//				parameters.set("rotation", 0); // 镜头角度转90度（默认摄像头是横拍） 
	...
	protected synchronized void updateCamera() throws RuntimeException {
		...
		if (mQuality.framerate >= max[0] && mQuality.framerate <= max[1])
		{
