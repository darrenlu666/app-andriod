...
public abstract class AudioStream  extends MediaStream {
	...
	{
		if (mMode == MODE_LIBFAAC_API && mPacketizer != null)
		{
			((RTMPSoftAACPacketizer)mPacketizer).DeliverAudioPlayData(data, length);
		}
	}
	...
	public void SetAecm(boolean isAecm) 
  	{
		if (mMode == MODE_LIBFAAC_API && mPacketizer != null)
  		{
  			((RTMPSoftAACPacketizer)mPacketizer).SetAecm(isAecm);
  		}
	}
	...
	public void SetNs(boolean isNs)
	{
		if (mMode == MODE_LIBFAAC_API && mPacketizer != null)
  		{
			((RTMPSoftAACPacketizer)mPacketizer).SetNs(isNs);
  		}
	}
	...
	public void SetAgc(boolean isAgc) 
	{
		if (mMode == MODE_LIBFAAC_API && mPacketizer != null)
  		{
			((RTMPSoftAACPacketizer)mPacketizer).SetAgc(isAgc);
  		}
	}
	// end
	...
	protected void encodeWithMediaRecorder() throws IOException {
		...
		Log.d("time", String.format("MediaRecorder startTime=%d", startTime));
