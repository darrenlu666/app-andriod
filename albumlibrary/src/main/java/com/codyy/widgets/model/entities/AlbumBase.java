...
public class AlbumBase {
    ...
    public static void selectInit(@NonNull ArrayList<PhotoInfo> infos) {
        ...
        for (PhotoInfo photoInfo : SELECT_INFO) {
            if (photoInfo.ismCheck()) {
                MAX_INDEX = photoInfo.getmPosition();
            }
        }
    }
    ...
    private static boolean removeItem(PhotoInfo info) {
        for (PhotoInfo info1 : SELECT_INFO) {
            if (info.getPath().equals(info1.getPath())) {
                SELECT_INFO.remove(info1);
                return true;
            }
        }
        return false;
     ...
     * @param info
     */
    public static boolean onPhotoSelect(PhotoInfo info) {
        if (info.ismCheck()) {
                ...
                if (size > 0 && size <= MAX_SIZE) {
                    ...
                    for (PhotoInfo info : SELECT_INFO) {
                        if (info.getPath().equals(photoInfo.getPath())) {
                            SELECT_INFO.add(info.getmPosition() - 1, photoInfo);
                            SELECT_INFO.remove(info);
                            break;
                        }
                    }
